...
import { drawChart } from './analyzator.js'
    ...
    (async () => {
        ...
        if (process.env.TWITTER_ENABLED === 'true' || process.env.TWITTER_ENABLED === true) {
            Object.keys(tweets).forEach(exchange => {
                ...
                tweets[exchange].imagePath = `screenshots/${exchange.replaceAll('io', '')}.jpeg`
                console.log('tweets[exchange]', tweets[exchange])

                tweet(tweets[exchange].text, tweets[exchange].imagePath)
            })
        }
